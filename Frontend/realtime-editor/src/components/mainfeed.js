...
function App() {
    ...
    if (token) {
      try {
        const res = await axios.post(
          `http://51.103.213.89:8000/dc/share/${selectedDocId}`,
          {
  ...
  async function fetchOwnedDocuments() {
    ...
    if (token) {
      try {
        const res = await axios.get(`http://51.103.213.89:8000/dc/viewAll`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  async function fetchSharedDocuments() {
    ...
    if (token) {
      try {
        const res = await axios.get(`http://51.103.213.89:8000/dc/viewShared`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  async function handleDelete(documentId) {
    ...
    try {
      const res = await axios.delete(
        `http://51.103.213.89:8000/dc/delete/${documentId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  async function handleRename(documentId, newName) {
    ...
    try {
      const res = await axios.put(
        `http://51.103.213.89:8000/dc/rename/${documentId}`,
        { title: newName },
        {
          headers: {
            Authorization: `Bearer ${token}`,
